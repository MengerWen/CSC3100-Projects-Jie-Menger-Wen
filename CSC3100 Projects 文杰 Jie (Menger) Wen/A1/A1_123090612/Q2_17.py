    ...
    for split in splits[1:]:
        ...
        while stack and not (a >= stack[-1][0] and b <= stack[-1][1]):
            ...
            if len(children[parent]) == 1:
                ...
                # 保留原有条件，允许检查子区间与父区间不完全相同的情况
                if child_interval != parent:
                    # 获取父区间的左、右端点
                    p_left, p_right = parent
                    # 获取子区间的左、右端点
                    c_left, c_right = child_interval
                    # 增加新的判断条件，允许子区间通过扩展左右端点与父区间重合
                    if not ((c_left == p_left and c_right + 1 == p_right) or
                            (c_left - 1 == p_left and c_right == p_right)):
                        print(0)
                        return
